...
public class MinLatEDPSegmentModel {

	private GRBModel model;
	private GRBVar[][][] x;
	private Graph g;
	private ForwGraphs forw;
	private int s, t, maxSeg, D, P;
	private boolean integral;

	public MinLatEDPSegmentModel(Graph g, int s, int t, int maxSeg, boolean integral) {
		this.g = g;
		...
		MaxSrEDPSegmentModel segment = new MaxSrEDPSegmentModel(g, s, t, maxSeg, integral);
		P = (int)segment.optimize();
		buildModel();
	}
	...
	public void buildModel() {
		try {
			...
			for(int u = 0; u < g.V(); u++) {
				for(int v = 0; v < g.V(); v++) {
					for(int d = 0; d < D; d++) {						
						...
						x[d][u][v] = model.addVar(0, zero ? 0 : 1, forw.getForwLat(u, v), integral ? GRB.INTEGER : GRB.CONTINUOUS, "x[" + u + "," + v + "," + d + "]");
					}
				}
			}
			...
			model.addConstr(expr, GRB.EQUAL, P, "number of paths");
			model.update();
