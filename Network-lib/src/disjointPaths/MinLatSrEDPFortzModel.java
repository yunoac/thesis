...
public class MinLatSrEDPFortzModel {
	...
	public MinLatSrEDPFortzModel(Graph g, int s, int t, int K, boolean integral) {
		this.g = g;
		...
		MaxSrEDPFortzModel fortz = new MaxSrEDPFortzModel(g, s, t, K, integral);
		P = (int)fortz.optimize();
		buildModel();
	}
	...
	public void buildModel() {
		try {
			...
			model.update();
			// add number of path constraints
			GRBLinExpr expr = new GRBLinExpr();
			for(int u = 0; u < g.V(); u++) {
				if(u == s) continue;
